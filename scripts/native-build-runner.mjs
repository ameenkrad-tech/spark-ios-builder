...
const GEN_SYSTEM =
  ...
  "FRAMEWORKS: SwiftUI + Foundation everywhere. App/Widget/Shared MAY use MapKit (iOS-17 `Map`), CoreLocation (then declare `CAPABILITIES: location`), Swift `Charts`, ActivityKit + WidgetKit, and AVFoundation's `AVSpeechSynthesizer` for spoken text-to-speech / narration (playback needs no permission). Watch: SwiftUI + Foundation (AVSpeechSynthesizer OK). Do NOT use SwiftData, push notifications, image/asset files, or external packages.\n\n" +
  "NETWORKING: you MAY use `URLSession` (async/await) to fetch REAL data from FREE, KEYLESS public HTTPS APIs when it genuinely improves the app — e.g. Open-Meteo `https://api.open-meteo.com/v1/forecast?...` for weather/forecasts, or other public no-auth JSON. NEVER use API keys, tokens, auth, or paid services (they will not work). CRITICAL: the app is screenshotted a few seconds after launch, so ALWAYS render sensible BUILT-IN placeholder/sample data IMMEDIATELY on first paint, then refresh it from the network in the background — NEVER show only a spinner, and fall back to the placeholder on any error. Decode with `Codable`; keep it resilient.\n\n" +
...
export function buildAttempt(files, { displayName = "App", capabilities = [], icon = null } = {}) {
  ...
  try {
    ...
    execSync("sleep 6"); // let the first frame render (+ a beat for any network fetch)
